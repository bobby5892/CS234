...
namespace CustomerMaintenance
{
    public partial class Form1 : Form
    {
        ...
        private void fillComboButton_Click(object sender, EventArgs e)
        {
           ...
           // List<Customer> customers = MMABooksEntities.entities.Customers.OrderBy(c => c.Name).ToList();
        ...
        private void fillGridButton_Click(object sender, EventArgs e)
        {
           ...
           // var customers = (from c in MMABooksEntities.entities.Customers orderby c.Name select new { c.CustomerID, c.Name }).ToList();
          //  var customers = MMABooksEntities.entities.Customers.Where(c => c.State == "NY").OrderBy(c => c.Name).Select(c => new { c.CustomerID, c.Name }).ToList();
            // *** Use both syntax to combine customer and state information
           /* var customers = (from c in MMABooksEntities.entities.Customers 
                             join s in MMABooksEntities.entities.States
                             on c.State equals s.StateCode
                             orderby c.Name select new { c.CustomerID, c.Name, c.State, s.StateName }).ToList();
    */      
    var customers = MMABooksEntities.entities.Customers.Join(
                MMABooksEntities.entities.States,
                c => c.State,
                s => s.StateCode,
