...
/*-- 10. Get me a list of all of the invoices.  
	Include all of the items ordered on the invoice, including the detailed information about each product.
--     You'll have more than one record for each invoice.*/
	SELECT Invoices.InvoiceID,Products.Description,
		InvoiceLineItems.ProductCode,InvoiceLineItems.Quantity
...
/*-- 11. Add the customer's name and address to the results from 10.*/
	SELECT Customers.Name, Invoices.InvoiceID,Products.Description,
		InvoiceLineItems.ProductCode,InvoiceLineItems.Quantity
	FROM  InvoiceLineItems
	INNER JOIN Invoices
		ON InvoiceLineItems.InvoiceID=Invoices.InvoiceID
...
/*-- 12. Add the name of the state to the results from 11.*/
	SELECT Customers.Name,States.StateName, Invoices.InvoiceID,Products.Description,
		InvoiceLineItems.ProductCode,InvoiceLineItems.Quantity
	 FROM  InvoiceLineItems
		INNER JOIN Invoices
	ON InvoiceLineItems.InvoiceID=Invoices.InvoiceID
...
/*-- 13. How many products do we have?*/
	SELECT COUNT(*) AS 'TotalSKUs' FROM Products
/*-- 14. What's the total value of all of the products sold?*/
	SELECT SUM(UnitPrice*Quantity) AS 'totalValue' FROM InvoiceLineItems
...
/*-- 16. How many orders has each customer placed?  EXTRA CREDIT:  
List all customers, even if they don't have any orders.*/
	SELECT Customers.Name,COUNT(Invoices.CustomerID) AS 'TotalOrders' FROM Customers
		LEFT JOIN Invoices
		ON Customers.CustomerID = Invoices.CustomerID
